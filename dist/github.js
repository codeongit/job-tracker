...
export function validateConfig(config) {
  if (!/^[\w.-]+$/.test(config.owner) || !/^[\w.-]+$/.test(config.repo))
    throw new Error('请填写有效的 GitHub 用户名和仓库名。');
  if (
    !config.path ||
    config.path.startsWith('/') ||
    config.path.split('/').some((p) => !p || p === '.' || p === '..') ||
    !config.path.endsWith('.json')
  )
    throw new Error('数据路径必须是仓库内的 JSON 文件路径。');
  return config;
}
export function githubClient(config, token, fetcher = fetch) {
  validateConfig(config);
  if (!token.trim()) throw new Error('请先在设置中填写本次会话使用的 GitHub 令牌。');
  ...
  async function request(path, options = {}, raw = false) {
    ...
    try {
      ...
      throw new RemoteError('无法连接 GitHub；本地修改仍然保留。请联网后重试。', 0);
    }
    if (!response.ok) {
      const messages = {
        401: '令牌无效或已过期。',
        403: '没有读写权限或请求过于频繁，请检查权限后稍后重试。',
        404: '仓库、分支或数据文件不存在，或当前令牌无权访问。',
        409: '云端刚刚有更新，请重新同步。',
        422: 'GitHub 未接受这次更新，请检查文件及写入权限。',
      };
    ...
    for (;;) {
      ...
      if (size > MAX_SYNC_BYTES) {
        await reader.cancel();
        throw new RemoteError('云端数据超过 5 MB，已停止同步。', 413);
      }
      chunks.push(value);
    }
  ...
  async function verify() {
    ...
    if (!branch) throw new Error('仓库尚未初始化，请先在 GitHub 创建 README。');
    // Verify the branch independently: a contents 404 can then be treated as a missing file.
    await request(`/branches/${encodeURIComponent(branch)}`);
    return branch;
  }
  ...
  async function read() {
    ...
    try {
      ...
      if (
        ...
        throw new Error('云端数据文件过大或格式不支持，已停止同步。');
      let data;
      try {
        // Blob SHA pins the raw response even when the branch changes between requests.
        const text =
          file.encoding === 'none'
      ...
      if (e.status === 404) return { data: emptyData(), sha: null, missing: true };
      throw e;
    }
  }
  async function write(data, sha) {
    const text = serializeForSync(data);
    if (!branch) await verify();
    const body = { message: 'Update job tracker data', content: encodeUtf8(text), branch };
    if (sha) body.sha = sha;
...
export function acknowledge(current, captured, uploaded) {
  ...
  return {
    ...
    pending: rebased.conflicts.length
      ? { data: rebased.data, conflicts: rebased.conflicts, generation: current.generation }
      : null,
  };
}
export async function syncWorkspace({ client, readState, updateState, allowCreate = false }) {
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (remote.missing && !equal(captured.base, emptyData()))
      throw new Error('原有云端文件消失了，请检查仓库或恢复文件，系统不会自动重建覆盖。');
    ...
    return {
      ...
      dirty: !equal(updated.data, updated.base),
    };
  }
