import { MAX_REQUEST_BYTES } from '../dist/limits.js';
...
export async function handleLocalApi(req, res, { port, secret, bridge, target }) {
  ...
  if (path === '/__local/session') {
    ...
    json(res, 200, {
      enabled: !!bridge,
      session: bridge ? secret : undefined,
  ...
  try {
    ...
    for await (const chunk of req) {
      size += chunk.length;
      if (size > MAX_REQUEST_BYTES) throw new BridgeError('数据超过 5 MB。', 413);
      chunks.push(chunk);
    }
    let body;
    try {
    ...
    json(res, error instanceof BridgeError ? error.status : 400, {
      message:
        error instanceof BridgeError ? error.message : '数据校验或本地操作失败，已停止同步。',
      code: error instanceof BridgeError ? error.code : 'DATA_INVALID',
    });
  }
  return true;
}
