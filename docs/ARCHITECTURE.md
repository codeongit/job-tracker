...
每日记录使用 `daily.js` 按明确日期筛选，再按岗位合并当天事件，避免首次联系、沟通和行动重复显示公司信息。计划使用 `dueAt`，完成和取消使用浏览器本地日期下的 `completedAt`，沟通使用 `activities.date`，首次联系使用 `appliedAt`。历史页显示当前岗位信息供识别，不推断所选日期当天的历史阶段。
